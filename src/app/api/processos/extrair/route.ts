...
import { extrairDadosComGemini } from "@/lib/gemini-extract";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[extrair] PDF recebido: "${arquivo.name}", ${(buffer.length / 1024).toFixed(0)} KB`);
  } catch (err) {
    console.error("[extrair] erro ao ler bytes:", err);
    return NextResponse.json(
  ...
  let resultado;
  try {
    resultado = await extrairDadosComGemini(buffer);
  } catch (err: unknown) {
    console.error("[extrair] erro no Gemini:");
    console.error(err);
  ...
  return NextResponse.json({
    dados,
    // Em desenvolvimento, inclui a resposta bruta do Gemini para debug
